...
landc_matrix <- read_excel("landcover_matrix.xlsx")
...
landc_matrix <- merge(landc_matrix, Aridity, by = "Sample.name")

landc_matrix$Aridity <- (1 - landc_matrix$AI)
...
landc_matrix$rep <- sub(".*(-\\w)", "\\1\\2", landc_matrix$Sample.name)
landc_matrix <- landc_matrix[, c("rep", setdiff(names(landc_matrix), "rep"))]

landc_matrix$Site.rep <- paste(landc_matrix$Site, landc_matrix$rep, sep = "")
landc_matrix <- landc_matrix[, c("Site.rep", setdiff(names(landc_matrix), "Site.rep"))]





landc_matrix <- landc_matrix[,-c(2:7,15)]

landc_matrix <- as.data.frame(landc_matrix)
rownames(landc_matrix) <- landc_matrix$Site.rep

landc_matrix <- landc_matrix[order(landc_matrix$Aridity), ]

landc <- landc_matrix[,-c(1,9)]

rm(Aridity, landc_matrix)
...
# !!!! 16S vs Env: Without landcover ----
rda10 <- rda(prok_h ~ ., data=data.frame(env1_s))
summary(rda10)

R2 <- RsquareAdj(rda10)$r.squared # Aquest és el valor que ja havíem vist abans al summary
R2
R2adj <- RsquareAdj(rda10)$adj.r.squared # Aquests él el valor ajustat que ens interessa
R2adj


anova.cca(rda10, permutations = how(nperm = 10000))
# significative
anova.cca(rda10, by = "axis", permutations = how(nperm = 1000))
# 3 RDA significative



plot(rda10, scaling=2, main='Triplot - scaling 2')
...
func_s_scores <- vegan::scores(rda10, display = "bp")
func_s_scores_df <- as.data.frame(func_s_scores)
rm(func_s_scores)

site_scores <- vegan::scores(rda10, display = "sites") 
site_scores_df <- as.data.frame(site_scores)
rm(site_scores)
site_scores_df$SiteRowNames <- rownames(site_scores_df)
site_scores_df$Site <- rownames(site_scores_df)
...
species_scores <- vegan::scores(rda10, display = "species")
species_scores_df <- as.data.frame(species_scores)
species_scores_df$var <- rownames(species_scores_df)
rm(species_scores)
...
# ggsave(path = "C:/Users/ecologia.PCECO002/OneDrive - Universitat de Girona/GRADCATCH/Manuscripts/1 GRADIENT/Figures",
#        "RDA_16S_ENV_NOLANDC.png", width = 10, height = 8, dpi = 300)
...
func_s_scores_df_sig <- func_s_scores_df[c(9,30,7,15,12,5,18,11,24,4,29,36,2),]
env1_s1 <- env_s[,c(9,30,7,15,12,5,18,11,24,4,29,36,2)]


rownames(func_s_scores_df_sig)[3] <- "WC"
rownames(func_s_scores_df_sig)[6] <- "STemp"
rownames(func_s_scores_df_sig)[9] <- "s350-400"
...
# !!!! ITS2 vs Env: Without landcover ----
rda40 <- rda(ITS2_h ~ ., data=data.frame(env1_s))
summary(rda40)

R2 <- RsquareAdj(rda40)$r.squared # Aquest és el valor que ja havíem vist abans al summary
R2
R2adj <- RsquareAdj(rda40)$adj.r.squared # Aquests él el valor ajustat que ens interessa
R2adj


anova.cca(rda40, permutations = how(nperm = 10000))
# significative
anova.cca(rda40, by = "axis", permutations = how(nperm = 1000))
# 4 RDA significative



plot(rda40, scaling=2, main='Triplot - scaling 2')
...
func_s_scores <- vegan::scores(rda40, display = "bp")
func_s_scores_df <- as.data.frame(func_s_scores)
rm(func_s_scores)

site_scores <- vegan::scores(rda40, display = "sites") 
site_scores_df <- as.data.frame(site_scores)
rm(site_scores)
site_scores_df$SiteRowNames <- rownames(site_scores_df)
site_scores_df$Site <- rownames(site_scores_df)
...
species_scores <- vegan::scores(rda40, display = "species")
species_scores_df <- as.data.frame(species_scores)
species_scores_df$var <- rownames(species_scores_df)
rm(species_scores)
...
# ggsave(path = "C:/Users/ecologia.PCECO002/OneDrive - Universitat de Girona/GRADCATCH/Manuscripts/1 GRADIENT/Figures",
#        "RDA_ITS2_ENV_NOLANDC.png", width = 10, height = 8, dpi = 300)
...
func_s_scores_df_sig <- func_s_scores_df[c(9,25,3,16,36,4,15,35,13,17,29,5,20),]
env1_s1 <- env1_s[,c(9,25,3,16,36,4,15,35,13,17,29,5,20)]


rownames(func_s_scores_df_sig)[6] <- "Altitude"
rownames(func_s_scores_df_sig)[12] <- "STemp"
rownames(func_s_scores_df_sig)[8] <- "Peak B"
...
# !!!! Func vs Env: Without biomass & landcover ----

rda9 <- rda(func2_s ~ ., data=data.frame(env1_s))
summary(rda9)

R2 <- RsquareAdj(rda9)$r.squared # Aquest és el valor que ja havíem vist abans al summary
R2
R2adj <- RsquareAdj(rda9)$adj.r.squared # Aquests él el valor ajustat que ens interessa
R2adj


anova.cca(rda9, permutations = how(nperm = 10000))
# significative
anova.cca(rda9, by = "axis", permutations = how(nperm = 1000))
# 3 first RDA significatives



plot(rda9, scaling=2, main='Triplot - scaling 2')
...
func2_s_scores <- vegan::scores(rda9, display = "bp")
func2_s_scores_df <- as.data.frame(func2_s_scores)
rm(func2_s_scores)

site_scores <- vegan::scores(rda9, display = "sites") 
site_scores_df <- as.data.frame(site_scores)
rm(site_scores)
site_scores_df$SiteRowNames <- rownames(site_scores_df)
site_scores_df$Site <- rownames(site_scores_df)
...
species_scores <- vegan::scores(rda9, display = "species")
species_scores_df <- as.data.frame(species_scores)
species_scores_df$var <- rownames(species_scores_df)
rm(species_scores)
...
# ggsave(path = "C:/Users/ecologia.PCECO002/OneDrive - Universitat de Girona/GRADCATCH/Manuscripts/1 GRADIENT/Figures",
#        "RDA_FUNC_ENV_NOBIOMASS_NOLANDC.png", width = 10, height = 8, dpi = 300)
...
# order matrixes data: RESPONSE, EXPLANATORI MATRIXES
forward.sel(func2_s, env1_s,
            R2thresh = 0.99,
            adjR2thresh = 0.99,
            nperm = 999,
            R2more = 0.01, # This means that the forward selection will only add variables that explain at least 1% more variance to the model.
            alpha = 0.05) # The significance level used for testing each variable's inclusion into the model.
ordistep(rda9,direction="forward",perm.max = 200)


func2_s_scores_df_sig <- func2_s_scores_df[c(9,11,18,5,36,10,22,7,21),]
env1_hs <- env1_s[,c(9,11,18,5,36,10,22,7,21)]


rownames(func2_s_scores_df_sig)[4] <- "STemp"
rownames(func2_s_scores_df_sig)[7] <- "LTN"
...
env$landcover_n <- as.factor(env$landcover_n)

env <- env %>%
  mutate(landcover_n = case_when(
    landcover_n == "5" ~ "Mixed Forests",
    landcover_n == "7" ~ "Open Shrublands",
    landcover_n == "16" ~ "Barren",
    landcover_n == "4" ~ "Deciduous Broadleaf Forests",
    landcover_n == "9" ~ "Savannas",
    landcover_n == "10" ~ "Grasslands",
    landcover_n == "1" ~ "Evergreen Needleleaf Forests",
  ))
env$landcover_n <- as.factor(env$landcover_n)

# Assuming 'env' is your original environmental data
continuous_vars <- env[, sapply(env, is.numeric)]
categorical_vars <- env[, sapply(env, is.factor)]

# Scale continuous variables
continuous_vars_scaled <- scale(continuous_vars)

# Combine scaled continuous and categorical variables back into one data frame
env_sc <- data.frame(continuous_vars_scaled, categorical_vars)

env_sc <- env_sc %>%
  rename("Land.cover" = categorical_vars)
...
# Func vs Env: Without MB + Landcover as factor ----

env_sc$Land.cover <- relevel(as.factor(env_sc$Land.cover), ref = "Grasslands")  # Change to desired reference
#Faig això perquè en el forward.sel em surt Barren com a seleccionada i sinó no el puc triar

rda8 <- rda(func2_s ~ ., data=data.frame(env_sc))
summary(rda8)

R2 <- RsquareAdj(rda8)$r.squared # Aquest és el valor que ja havíem vist abans al summary
R2
R2adj <- RsquareAdj(rda8)$adj.r.squared # Aquests él el valor ajustat que ens interessa
R2adj


anova.cca(rda8, permutations = how(nperm = 10000))
# significative
anova.cca(rda8, by = "axis", permutations = how(nperm = 1000))
# 3 first RDA significatives



plot(rda8, scaling=2, main='Triplot - scaling 2')
...
func2_s_scores <- vegan::scores(rda8, display = "bp")
func2_s_scores_df <- as.data.frame(func2_s_scores)
rm(func2_s_scores)

site_scores <- vegan::scores(rda8, display = "sites") 
site_scores_df <- as.data.frame(site_scores)
rm(site_scores)
site_scores_df$SiteRowNames <- rownames(site_scores_df)
site_scores_df$Site <- rownames(site_scores_df)
...
species_scores <- vegan::scores(rda8, display = "species")
species_scores_df <- as.data.frame(species_scores)
species_scores_df$var <- rownames(species_scores_df)
rm(species_scores)
...
ggsave(path = "C:/Users/ecologia.PCECO002/OneDrive - Universitat de Girona/GRADCATCH/Manuscripts/1 GRADIENT/Figures",
       "RDA_FUNC_ENV_NOBIOMASS_LANDC.png", width = 10, height = 8, dpi = 300)



# Selection of the significant variables from the response matrix:

env_sc_numeric <- model.matrix(~ . - 1, data = env_sc)

# order matrixes data: RESPONSE, EXPLANATORI MATRIXES
forward.sel(func2_s, env_sc_numeric,
            R2thresh = 0.99,
            adjR2thresh = 0.99,
            nperm = 999,
            R2more = 0.01, # This means that the forward selection will only add variables that explain at least 1% more variance to the model.
            alpha = 0.05) # The significance level used for testing each variable's inclusion into the model.
ordistep(rda8,direction="forward",perm.max = 200)


func2_s_scores_df_sig <- func2_s_scores_df[c(9,11,18,37,40,39,2,21,10),]
env_sc_sel <- env_sc_numeric[,c(9,11,18,41,40,38,2,21,10)]
...
# ggsave(path = "C:/Users/ecologia.PCECO002/OneDrive - Universitat de Girona/GRADCATCH/Manuscripts/1 GRADIENT/Figures",
#        "RDA_FUNC_ENV_sel_NOBIOMASS_LANDC.png", width = 10, height = 8, dpi = 300)

rda81 <- rda(func2_s ~ ., data=data.frame(env_sc_sel))
summary(rda81)

R2 <- RsquareAdj(rda81)$r.squared # Aquest és el valor que ja havíem vist abans al summary
R2
R2adj <- RsquareAdj(rda81)$adj.r.squared # Aquests él el valor ajustat que ens interessa
R2adj


anova.cca(rda81, permutations = how(nperm = 10000))
...
rda7 <- rda(func2_s ~ ., data=data.frame(landc))
summary(rda7)

landc_reordered <- landc %>%
  select(Grasslands, everything())

rda7 <- rda(func2_s ~ ., data=data.frame(landc_reordered))
summary(rda7)

R2 <- RsquareAdj(rda7)$r.squared # Aquest és el valor que ja havíem vist abans al summary
R2
R2adj <- RsquareAdj(rda7)$adj.r.squared # Aquests él el valor ajustat que ens interessa
R2adj

#Surt igual que posant-ho amb la funció capscale d'en Jonathan


anova.cca(rda7, permutations = how(nperm = 10000))
# significative
anova.cca(rda7, by = "axis", permutations = how(nperm = 1000))
# 3 first RDA significatives



plot(rda7, scaling=2, main='Triplot - scaling 2')
...
func2_s_scores <- vegan::scores(rda7, display = "bp")
func2_s_scores_df <- as.data.frame(func2_s_scores)
rm(func2_s_scores)

site_scores <- vegan::scores(rda7, display = "sites") 
site_scores_df <- as.data.frame(site_scores)
rm(site_scores)
site_scores_df$SiteRowNames <- rownames(site_scores_df)
site_scores_df$Site <- rownames(site_scores_df)
...
species_scores <- vegan::scores(rda7, display = "species")
species_scores_df <- as.data.frame(species_scores)
species_scores_df$var <- rownames(species_scores_df)
rm(species_scores)
...
ggsave(path = "C:/Users/ecologia.PCECO002/OneDrive - Universitat de Girona/GRADCATCH/Manuscripts/1 GRADIENT/Figures",
       "RDA_FUNC_LANDC.png", width = 10, height = 8, dpi = 300)
...
# order matrixes data: RESPONSE, EXPLANATORI MATRIXES
forward.sel(func2_s, landc_reordered,
            R2thresh = 0.99,
            adjR2thresh = 0.99,
            nperm = 999,
            R2more = 0.01, # This means that the forward selection will only add variables that explain at least 1% more variance to the model.
            alpha = 0.05) # The significance level used for testing each variable's inclusion into the model.
ordistep(rda7,direction="forward",perm.max = 200)


func2_s_scores_df_sig <- func2_s_scores_df[c(1,2,5,6),]
landc_reordered_s <- env_s[,c(1,2,5,6)]
...
ggsave(path = "C:/Users/ecologia.PCECO002/OneDrive - Universitat de Girona/GRADCATCH/Manuscripts/1 GRADIENT/Figures",
       "RDA_FUNC_LANDC_sel.png", width = 10, height = 8, dpi = 300)

rda71 <- rda(func2_s ~ ., data=data.frame(landc_reordered_s))
summary(rda71)

R2 <- RsquareAdj(rda71)$r.squared # Aquest és el valor que ja havíem vist abans al summary
R2
R2adj <- RsquareAdj(rda71)$adj.r.squared # Aquests él el valor ajustat que ens interessa
R2adj
