## Models for the functions on the paper "Soil functions are shaped by aridity through soil properties and the microbial community structure -----------
## Created by Luciana Chavez Rodriguez
## Edited by Anna Doménech-Pascual
# May 2024
...
# Load and edit data ----
my_data <- read_delim("/INPUT_DATA/my_data_models.csv", 
...
max_values      = apply(my_data[,c(7:26,36,40:52)],2,max)
my_data.1       = as.data.frame(cbind(my_data[,c(1,55,27:35,53:54)],
                                      my_data[,c(7:26,36,40:52)] / as.list(max_values)))
a.1             = as.data.frame(colnames(my_data.1))

corr            = as.data.frame(cor(my_data.1[,12:35]))

my_data.1 <-my_data.1 %>%
  mutate(Cenz = select(., 3:6) %>% rowSums(na.rm = TRUE)) %>% 
  mutate(MB = select(.,32:33) %>% rowSums(na.rm = TRUE))


# VIF ----
my_data_vif = as.data.frame(cbind(my_data.1[,c(2,14,16:47)]))
vif(my_data_vif)
vifcor(my_data_vif, th=0.9)
vifstep(my_data_vif, th=10)
...
# NAG ----
# nag (N-acetyl-β-glucosaminidase) is abbreviated as gla in this dataset
...
# PHOS ----
# phos is abbreviated as fos in this dataset
