...
mit_16S <- read_excel("C:/Users/ecologia.PCECO002/OneDrive - Universitat de Girona/GRADCATCH/ANALISIS/amplicon_sequencing_2021_JD/16S/Alphadiv_SP_16S_mit.xlsx")
mit_18S <- read_excel("C:/Users/ecologia.PCECO002/OneDrive - Universitat de Girona/GRADCATCH/ANALISIS/amplicon_sequencing_2021_JD/ITS2/Alphadiv_SP_ITS2_mit.xlsx")
...
mit_16S$Aridity <- (1 - mit_16S$AI)
mit_18S$Aridity <- (1 - mit_18S$AI)
...
# FUNCTIONS ----
#Clean non-interesting variables:
cor_data <- my_data[-c(61:63),c(2,7,40,41,46:53,64)]
...
for (var in numerical_variables_names) {
  if (var != "Aridity") {
    plot_title <- paste("Aridity vs", var)
    all_plots[[length(all_plots) + 1]] <- ggscatter(mit_16S, x = "Aridity", y = var,
                                                    add = "reg.line", conf.int = TRUE,
                                                    cor.coef = TRUE, cor.method = "spearman",
                                                    xlab = "Aridity", ylab = var,
                                                    main = plot_title)
...
for (var in numerical_variables_names) {
  if (var != "Aridity") {
    plot_title <- paste("Aridity vs", var)
    all_plots[[length(all_plots) + 1]] <- ggscatter(mit_18S, x = "Aridity", y = var,
                                                    add = "reg.line", conf.int = TRUE,
                                                    cor.coef = TRUE, cor.method = "spearman",
                                                    xlab = "Aridity", ylab = var,
                                                    main = plot_title)
