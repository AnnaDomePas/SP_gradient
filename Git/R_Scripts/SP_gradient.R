## TITLE: ..........-----------
## Created by Anna Doménech-Pascual, May 2024
...
# Load functions ----
# Data_summary (calculates Mean and SD):
...
# Data_summary2 (calculates Mean, SD and SE)
...
mit_16S <- read_excel("Alphadiv_SP_16S_mit.xlsx")
mit_18S <- read_excel("Alphadiv_SP_ITS2_mit.xlsx")


#Creating variable Aridity (1-AI):
...
# Changing names of variables for easier interpretation:
...
#Adding column with aridity values into "mit_16S" and "mit_18S" datasets:
...
# Choose the dataset you are interested on and then run
# the following lines (lines 103 to 150):
...
# To test correlationships between SP06
# (for Physicochemical and Functional variables) run line 100:
...
# > 1. PCA plot (Soil Organic Matter composition) ----
...
# > 2. Remove redundant variables (caret package) ----
...
New_data$enzyme <- factor(New_data$enzyme, levels = c("alpha", "beta", 
                                                      ...
                                                      "leu","phe" ))
New_data <- data_summary2(New_data, varname="values", 
                          groupnames=c("Site","aridity", "enzyme"))
New_data$values = as.numeric(New_data$values)

enzyme <- ggplot(New_data, aes(x=aridity, y=values)) +
...
ggsave(path = "Figures/1 GRADIENT", "SP_enzymes_aridity_se2.png", width = 16, height = 8, dpi = 300)
...
New_data$values = as.numeric(New_data$values)
respiration <- ggplot(New_data, aes(x=aridity, y=values, color = respi, 
                                    fill = respi)) +
  geom_point(size= 3, color ="grey") +
  geom_pointrange(data = New_data, aes(ymin=values-sd, ymax=values+sd),
                  color = "black", stroke = 1, size = 0.6, shape = 1) +
